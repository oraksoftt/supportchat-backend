...
public class ChatHub : Hub
{
    ...
    public async Task CloseChat(long chatId)
    {
...
//    // Safely check if the connection is an authenticated agent/internal user
//    private bool IsAgentConnection => Context.User?.Identity?.IsAuthenticated ?? false;
...
//                // Validate if agent is still active in system
//                var valid = await _authService.ValidateUserAsync(userId);
//                if (!valid)
//                {
//                    _logger.LogWarning("Rejected connection for inactive agent {UserId}", userId);
...
//                await _connectionManager.AddConnectionAsync(userId, connectionId);
//                try { await _authService.SetOnlineStatusAsync(userId, true); } catch { /* best-effort */ }

//                // Group for internal broadcast streams (all internal dashboard agents)
//                await Groups.AddToGroupAsync(connectionId, $"company-{companyId}");
...
//            else
//            {
//                // Anonymous customers are allowed to connect without initial user mapping.
//                // Their tracking links directly inside JoinCustomerChat invocation.
//                _logger.LogInformation("Anonymous customer client connected. ConnectionId: {ConnectionId}", connectionId);
//            }
...
//            else
//            {
//                // Clean up any loose structures map track patterns for anonymous clients if required by your ConnectionManager
//                await _connectionManager.RemoveConnectionAsync(currentConnId);
//                _logger.LogInformation("Anonymous customer disconnected: {ConnectionId}", currentConnId);
//            }
//        }
...
//            // Strict context validation matching identity type
//            if (dto.SenderType == SenderType.Agent)
//            {
//                if (!IsAgentConnection) throw new HubException("Invalid system configuration permissions.");
//                senderId = GetUserId();
...
//            else if (dto.SenderType == SenderType.Customer)
//            {
//                // Fetch context implicitly via the targeted chat configuration record 
//                var chat = await _chatService.GetByIdAsync(dto.ChatId) ?? throw new HubException("Chat target unavailable.");
//                companyId = chat.CompanyId;

//                // If it's a customer, senderId is tracking properties on the schema context layer or left null
//                senderId = chat.CustomerId;
//            }
//            else
//            {
...
//            if (message.CreatedOn.Kind == DateTimeKind.Unspecified)
//            {
//                message.CreatedOn = DateTime.SpecifyKind(
//                    message.CreatedOn,
//                    DateTimeKind.Utc
//                );
//            }
...
//            // Notify everyone listening directly inside the active conversation room
//            await Clients.Group($"chat-{dto.ChatId}").SendAsync("ReceiveMessage", message);

//            // Feed global enterprise workspace feed tracking monitors
...
//        try
//        {
//            // Fallback gracefully for identification keys inside typing state notifications
//            long trackingId = IsAgentConnection ? GetUserId() : 0;
//            await Clients.Group($"chat-{chatId}").SendAsync("Typing", trackingId, isTyping);
//        }
//        catch (Exception ex)
